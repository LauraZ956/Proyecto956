...
$old_error_handler = set_error_handler("calendario_sedes_scriptcase_error_handler");

function calendario_sedes_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['calendario_sedes']['contr_erro']) && $_SESSION['scriptcase']['calendario_sedes']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_calendario_sedes->contr_calendario_sedes->Apl_com_erro = true;
            if ($inicial_calendario_sedes->contr_calendario_sedes->NM_ajax_flag)
            {
                $inicial_calendario_sedes->contr_calendario_sedes->NM_ajax_info['errList']['geral_calendario_sedes'][] = $msg;
                return;
            }
            calendario_sedes_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function calendario_sedes_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['calendario_sedes'])  && '' != $_SESSION['scriptcase']['error_icon']['calendario_sedes'])  ? $_SESSION['scriptcase']['error_icon']['calendario_sedes']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['calendario_sedes']) && '' != $_SESSION['scriptcase']['error_close']['calendario_sedes']) ? $_SESSION['scriptcase']['error_close']['calendario_sedes'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['calendario_sedes']) && '' != $_SESSION['scriptcase']['error_span_title']['calendario_sedes']) ? $_SESSION['scriptcase']['error_span_title']['calendario_sedes'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['calendario_sedes']) && '' != $_SESSION['scriptcase']['error_icon_title']['calendario_sedes']) ? $_SESSION['scriptcase']['error_icon_title']['calendario_sedes'] : '';
?>
