...
$_SESSION['scriptcase']['app_Login']['contr_erro'] = 'off'; 
    ...
    function ValidateField_login(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          ...
          { 
              $hasError = true;
              $Campos_Crit .= "" . $this->Ini->Nm_lang['lang_sec_users_fild_login'] . " " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 200 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['login']))
              {
                  $Campos_Erros['login'] = array();
              }
...
;
	...
	if($this->sc_logged($this->login )):
        ...
        if (is_file($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_pasaporte_beta2.txt")) {
    unlink($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_pasaporte_beta2.txt");
}
if (is_file("../_lib/friendly_url/app_Login_ini.txt")) {
    $SC_arq_def = fopen($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_pasaporte_beta2.txt", "w");
    fwrite ($SC_arq_def, 'app_Login, M');
    fclose ($SC_arq_def);
    setcookie('sc_apl_default_pasaporte_beta2','app_Login, M','0','/');
}
;
		 if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('inicio') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };	
	endif;
}
if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
